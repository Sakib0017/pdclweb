...
const Hero = ({ color }) => {
  ...
  const handleBranchChange1 = (event) => {
    setSelectedBranch1(event.target.value);
    setFilteredDoctors([]); // Reset filtered services on branch change
    setSearchTerm1("");
  };
  ...
  const handleSearchChange1 = (event) => {
    ...
    const branchDoctors = doctorData.find(
      (branch) => branch.braId === parseInt(selectedBranch1)
    )?.doctors;
    if (branchDoctors) {
      const filtered = branchDoctors.flatMap((category) =>
        category.items.filter((doctor) =>
